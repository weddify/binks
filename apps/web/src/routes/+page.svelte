...
<div class="flex flex-col min-h-screen">
  ...
  <main class="flex flex-1 flex-col items-center px-4 md:px-8 pb-10">
    <div class="flex flex-col w-full max-w-[1200px] gap-12">
      ...
      <!-- Hero Carousel -->
      {#if isLoading}
        <Skeleton class="mt-8 w-full h-[540px] md:h-auto md:aspect-[21/9] rounded-2xl" />
      {:else}
      ...
      <section>
           ...
           <div class="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-4 hover:shadow-md transition-shadow">
             ...
             <div class="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-4 hover:shadow-md transition-shadow">
               ...
               <div>
                  ...
                  <p class="text-sm text-slate-500 dark:text-slate-400">Our support team is ready to help you whenever you need assistance.</p>
               </div>
            </div>
          </div>
        </section>
        ...
        <section>
           ...
           <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {#if isLoading}
                  <!-- Render 8 skeletons to simulate a full grid -->
                  {#each Array(8) as _}
                      <ProductCardSkeleton />
                  {/each}
              {:else}
        ...
        <section class="py-8 border-t border-slate-200 dark:border-slate-700/50">
           <div class="text-center mb-10">
              <h2 class="text-2xl font-bold mb-2">What They Say?</h2>
              <p class="text-slate-500 dark:text-slate-400">Thousands of happy customers trust our automated service.</p>
           </div>
           <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <!-- Testimonial 1 -->
